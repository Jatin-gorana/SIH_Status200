...
const Navbar = () => {
  ...
  const [userPhoto, setUserPhoto] = useState(''); // State to handle user photo
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (currentUser) => {
      ...
      if (currentUser) {
        try {
          // Fetch user role and photo from Firestore
          const userDoc = await getDoc(doc(db, 'users', currentUser.uid));
          if (userDoc.exists()) {
            const userData = userDoc.data();
            setUserRole(userData.role); // Assume 'role' field contains 'mentor' or 'mentee'
            setUserPhoto(userData.photoURL || ''); // Set photo URL from Firestore or empty string
          }
        } catch (error) {
  ...
  const profilePhoto = isGoogleSignIn ? user.photoURL : userPhoto || '/default-profile.png';

  return (
    <nav className="bg-black text-white p-4">
      <div className="container mx-auto flex justify-between items-center">
        <Link to="/">
          <img src={logo} alt="Logo" width="250px" height="160px" />
        </Link>
